...
<style>
    ...
    <p>TARIF GERBANG SISTEM TERTUTUP</p>
    <br>
        ...
        @foreach ($data as $item2)
            <tr>
                ...
                <td>
                    ...
                    @if ($item2->jenis == 'ags')
                        Ya
                    @elseif($item2->jenis == 'khl')
                        Tidak
                    @endif
                </center>
                </td>
                @foreach ($arrayValue as $key => $value)
                <td>{{ number_format(json_decode($item2->gol1_d)[$key], 0, ',', '.') }}</td>
                <td>{{ number_format(json_decode($item2->gol2_d)[$key], 0, ',', '.') }}</td>
                <td>{{ number_format(json_decode($item2->gol3_d)[$key], 0, ',', '.') }}</td>
                <td>{{ number_format(json_decode($item2->gol4_d)[$key], 0, ',', '.') }}</td>
                <td>{{ number_format(json_decode($item2->gol5_d)[$key], 0, ',', '.') }}</td>
                @endforeach
